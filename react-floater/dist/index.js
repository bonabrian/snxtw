...
function getModifiers(modifiers) {
  ...
  return (0, import_deepmerge_ts.deepmerge)(defaultOptions2, modifiers ?? {});
}
function isFixed(el) {
  if (!el) {
    return false;
...
function mergeModifier(modifier, customModifier) {
  return (0, import_deepmerge_ts.deepmerge)(modifier, customModifier ?? {});
}
...
function useEffectOnce(effect4) {
  (0, import_react2.useEffect)(effect4, []);
}
function useMount(effect4) {
  useEffectOnce(effect4);
}
function useSingleton(callback) {
  const hasBeenCalled = (0, import_react2.useRef)(false);
...
function useUnmount(fn2) {
  const fnRef = (0, import_react2.useRef)(fn2);
  fnRef.current = fn2;
...
function ReactFloaterPortal(props) {
  ...
  if (node.current) {
    ...
    return (0, import_react_dom.createPortal)(children, node.current);
  }
  return null;
}
...
function getStyles(styles) {
  ...
  return (0, import_deepmerge_ts2.deepmerge)(
    {
      arrow: {
        color: "#fff",
        display: "inline-flex",
      ...
      options: (0, import_deepmerge_ts2.deepmerge)(defaultOptions, options)
    },
    rest
  );
}
